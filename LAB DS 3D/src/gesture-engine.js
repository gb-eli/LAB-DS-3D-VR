...
export function analyzeHand(hand, {
  ...
  const fourFolded = !index.extended && !middle.extended && !ring.extended && !pinky.extended;
  const pinchConfidence = clamp(1 - pinchRatio / Math.max(0.001, pinchRelease), 0, 0.99);
  const candidateScores = {
    thumbs_up: thumbExtended && fourFolded && thumbDirection === "up" ? clamp(0.78 + (thumbAngle - 126) / 180, 0, 0.98) : 0,
    thumbs_down: thumbExtended && fourFolded && thumbDirection === "down" ? clamp(0.76 + (thumbAngle - 126) / 200, 0, 0.96) : 0,
    ok: pinching && middle.extended && ring.extended && pinky.extended ? clamp(0.66 + pinchConfidence * 0.32, 0, 0.99) : 0,
    pinch: pinching && !(middle.extended && ring.extended && pinky.extended) ? clamp(0.52 + pinchConfidence * 0.45, 0, 0.98) : 0,
    point: index.extended && !middle.extended && !ring.extended && !pinky.extended ? clamp(0.78 + index.pipAngle / 900, 0, 0.98) : 0,
    peace: index.extended && middle.extended && !ring.extended && !pinky.extended ? clamp(0.8 + (index.pipAngle + middle.pipAngle) / 1800, 0, 0.98) : 0,
    open: openScore > 0.45 ? clamp(0.5 + openScore * 0.47, 0, 0.98) : 0,
    fist: fourFolded ? clamp((fistScore * (thumbExtended ? 0.34 : 1)) + (thumbExtended ? 0 : 0.08), 0, 0.98) : 0
  };
  const alternatives = Object.entries(candidateScores)
    .map(([candidateType, score]) => ({ type: candidateType, score }))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score);
  const best = alternatives[0] || { type: "unknown", score: 0.42 };
  const runnerUp = alternatives[1]?.score || 0;
  const margin = Math.max(0, best.score - runnerUp);
  const type = best.score >= 0.54 ? best.type : "unknown";
  const confidence = type === "unknown" ? clamp(best.score, 0.3, 0.55) : clamp(best.score * 0.88 + margin * 0.32, 0.54, 0.99);
...
export class GestureEngine {
  ...
  update(hands = [], handedness = [], timestamp = performance.now(), worldHands = []) {
    ...
    const results = hands.map((hand, index) => {
      ...
      const framesNeeded = ["pinch", "fist", "ok", "thumbs_up", "thumbs_down"].includes(analysis.type) ? 2 : 3;
      const confidenceReady = analysis.type === "unknown" || analysis.confidence >= 0.58;
      if (confidenceReady && tracker.candidateFrames >= framesNeeded && analysis.type !== tracker.stable) {
