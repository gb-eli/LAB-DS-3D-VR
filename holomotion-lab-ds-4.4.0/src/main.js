...
class NullHoloScene {
  ...
  setDynamicPixelRatioScale() {} setColor() {} setObjectType() {} setAutoRotate() {} reset() {}
...
async function activateLazyMode(mode) {
  ...
  const instance = await moduleLoader.activate(mode, { callbacks: lazyCallbacks(mode), canvas: mode === "defender" ? $("#saberCanvas") : null });
  setLoading(false);
  return instance;
}
...
async function applyPerformanceConfiguration({ restartCamera = false, announce = false } = {}) {
  ...
  elements.performanceStatus.textContent = profile.label.toUpperCase();
  if (announce) showToast(`${profile.label} · ${performanceManager.usage.label}`);
  if (restartCamera && state.cameraOn) await vision.startCamera();
}
...
function modeNeedsHands() {
  return ["explorer", "sandbox", "catch", "draw", "gestures", "academy", "sequence", "aura", "saber", "libras", "checklist", "simon", "reflex", "marathon", "defender"].includes(state.mode) && !(state.mode === "catch" && state.players === 2);
}

function modeNeedsPose() {
  return ["sandbox", "catch", "pose", "academy", "sequence", "aura", "body", "dance", "stretch", "saber", "checklist", "simon", "reflex", "marathon", "defender"].includes(state.mode);
}
...
async function switchMode(mode, announce = true) {
  ...
  if (moduleLoader.has(mode)) await activateLazyMode(mode);
  bodyAnalyzer.reset();
...
function bindEvents() {
  ...
  $("#benchmarkButton").addEventListener("click", () => {
    const capability = performanceManager.detectCapability();
    const recommended = PERFORMANCE_PROFILES[capability.recommendation];
    showToast(`${recommended.label} recomendado · índice ${capability.score}`);
  });
  $("#optimizeButton").addEventListener("click", async () => {
    performanceManager.adaptiveLevel = Math.min(3, performanceManager.adaptiveLevel + 1);
    holoScene.setDynamicPixelRatioScale([1,.88,.76,.66][performanceManager.adaptiveLevel]);
...
function renderLoop(now) {
  ...
  if (state.started) {
    ...
    const hasRequiredTracking = state.mode === "face"
      ...
      : ["pose", "academy", "sequence", "aura", "body", "dance", "stretch", "saber", "checklist", "simon", "reflex", "marathon", "defender"].includes(state.mode) || (state.mode === "catch" && state.players === 2)
