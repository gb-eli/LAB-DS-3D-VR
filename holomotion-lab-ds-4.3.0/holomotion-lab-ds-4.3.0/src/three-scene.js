...
export class HoloScene {
  ...
  registerExplorerPart(part, explode = new THREE.Vector3()) {
  ...
  buildSolarSystem(root) {
    ...
    radii.forEach((radius, index) => {
      ...
      this.registerExplorerPart(pivot, new THREE.Vector3(Math.cos(index) * 0.45, (index - 3.5) * 0.08, Math.sin(index) * 0.45));
      if (index === 5) {
  ...
  buildEarthMoon(root) {
    ...
    this.registerExplorerPart(moonPivot, new THREE.Vector3(0.8, 0.3, 0));
    root.rotation.z = -0.2;
  }
  ...
  buildMotherboard(root) {
    ...
    const makePart = (geometry, color, position, explode) => {
      const mesh = new THREE.Mesh(geometry, this.explorerMaterial(color, 0.82));
      mesh.position.copy(position);
      root.add(mesh);
      this.addExplorerEdges(mesh, 0xe8fff8, 0.62);
      return this.registerExplorerPart(mesh, explode);
    };
    makePart(new THREE.BoxGeometry(1.15, 0.32, 1.15), 0x38bdf8, new THREE.Vector3(-0.75, 0.28, 0.25), new THREE.Vector3(-0.7, 0.75, 0.2));
    for (let i = 0; i < 4; i += 1) makePart(new THREE.BoxGeometry(0.16, 0.42, 1.65), 0xa78bfa, new THREE.Vector3(0.55 + i * 0.27, 0.31, -0.32), new THREE.Vector3(0.2 + i * 0.12, 0.85, -0.35));
    for (let i = 0; i < 3; i += 1) makePart(new THREE.BoxGeometry(1.85, 0.18, 0.15), 0xf8fafc, new THREE.Vector3(0.45, 0.26, 0.75 + i * 0.31), new THREE.Vector3(0.55, 0.65, 0.45 + i * 0.15));
    const chipset = makePart(new THREE.CylinderGeometry(0.44, 0.44, 0.24, 24), 0xf59e0b, new THREE.Vector3(-0.65, 0.28, -0.95), new THREE.Vector3(-0.5, 0.7, -0.6));
    chipset.rotation.x = Math.PI / 2;
    const fan = makePart(new THREE.CylinderGeometry(0.62, 0.62, 0.25, 32), 0x0f172a, new THREE.Vector3(-1.82, 0.32, -0.45), new THREE.Vector3(-0.85, 0.8, -0.3));
  ...
  buildDrone(root) {
    ...
    root.add(camera);
    this.registerExplorerPart(camera, new THREE.Vector3(0, -0.8, 0.65));
    ...
    coords.forEach((coord, index) => {
      ...
      arm.add(rotor);
      root.add(arm);
      this.registerExplorerPart(arm, new THREE.Vector3(coord[0] * 0.85, 0.35, coord[2] * 0.85));
  ...
  buildRobot(root) {
    ...
    const add = (geometry, color, position, explode) => {
    ...
    add(new THREE.BoxGeometry(1.35, 1.55, 0.72), 0x4f46e5, new THREE.Vector3(0, 0.35, 0), new THREE.Vector3(0, 0.3, 0));
    const core = add(new THREE.SphereGeometry(0.28, 18, 14), 0x22d3ee, new THREE.Vector3(0, 0.48, 0.4), new THREE.Vector3(0, 0.25, 0.8));
    this.explorerAnimated.push({ object: core, type: "pulse", speed: 1.7 });
    add(new THREE.BoxGeometry(0.88, 0.72, 0.72), 0x60a5fa, new THREE.Vector3(0, 1.65, 0), new THREE.Vector3(0, 0.9, 0));
    [-1, 1].forEach((side) => {
      add(new THREE.CapsuleGeometry(0.2, 1.05, 6, 10), 0x8b5cf6, new THREE.Vector3(side * 1.05, 0.45, 0), new THREE.Vector3(side * 0.8, 0.3, 0));
      add(new THREE.SphereGeometry(0.26, 16, 10), 0x22d3ee, new THREE.Vector3(side * 1.05, -0.32, 0), new THREE.Vector3(side * 0.95, -0.45, 0));
      add(new THREE.CapsuleGeometry(0.24, 1.15, 6, 10), 0x3b82f6, new THREE.Vector3(side * 0.46, -1.25, 0), new THREE.Vector3(side * 0.42, -0.9, 0));
    });
    root.scale.setScalar(0.92);
  }
  ...
  buildVolcano(root) {
    ...
    root.add(magma);
    this.registerExplorerPart(magma, new THREE.Vector3(0, 0.9, 0.65));
    ...
    root.add(chamber);
    this.registerExplorerPart(chamber, new THREE.Vector3(0, -0.7, 0));
  ...
  buildCave(root) {
    ...
    for (let i = 0; i < 9; i += 1) {
      ...
      root.add(crystal);
      this.registerExplorerPart(crystal, new THREE.Vector3((i % 5 - 2) * 0.18, 0.45, (Math.floor(i / 5) - 0.5) * 0.4));
  ...
  buildWorldMap(root) {
    ...
    shapes.forEach((item, index) => {
      ...
      root.add(land);
      this.registerExplorerPart(land, new THREE.Vector3(item[0] * 0.22, item[1] * 0.25, 0.7));
    });
    for (let i = 0; i < 7; i += 1) {
  ...
  buildLandmarks(root) {
    ...
    root.add(tower);
    this.registerExplorerPart(tower, new THREE.Vector3(-0.8, 0.2, 0));
    ...
    this.registerExplorerPart(pyramid, new THREE.Vector3(0.4, -0.2, 0.5));
    ...
    root.add(arch);
    this.registerExplorerPart(arch, new THREE.Vector3(0.85, 0.2, -0.2));
  ...
  bindPointer() {
    ...
    this.canvas.addEventListener("pointerdown", (event) => {
      if (!this.enabled || !["sandbox", "explorer"].includes(this.mode)) return;
      updatePointer(event);
      this.mouseDown = true;
  ...
  raycastObject() {
    ...
    return Boolean(target && this.raycaster.intersectObject(target, this.mode === "explorer").length);
  }
  ...
  setMode(mode) {
    this.cancelInteraction();
    this.mode = mode;
    this.enabled = mode === "sandbox" || mode === "explorer" || mode === "face";
    this.objectGroup.visible = mode === "sandbox";
    this.explorerGroup.visible = mode === "explorer";
    this.faceGroup.visible = mode === "face";
    this.rings.forEach((ring) => { ring.visible = mode === "sandbox" || mode === "explorer"; });
    this.grid.visible = this.enabled;
    this.particles.visible = this.enabled;
    this.canvas.style.pointerEvents = ["sandbox", "explorer"].includes(mode) ? "auto" : "none";
  ...
  animate(now) {
    ...
    if (this.mode === "sandbox") {
      ...
      this.explorerAnimated.forEach((item, index) => {
        ...
        if (item.type === "orbit") item.object.rotation.y += delta * item.speed;
        if (item.type === "spin") item.object.rotation.y += delta * item.speed;
        if (item.type === "spinReverse") item.object.rotation.y -= delta * item.speed;
        if (item.type === "spinY") item.object.rotation.y += delta * item.speed;
        if (item.type === "spinZ") item.object.rotation.z += delta * item.speed;
        if (item.type === "pulse") {
          const scale = 1 + Math.sin(elapsed * item.speed + index) * 0.06;
