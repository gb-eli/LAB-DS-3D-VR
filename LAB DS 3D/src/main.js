...
const elements = {
  ...
  blocksMaterialName: $("#blocksMaterialName"),
  blocksHint: $("#blocksHint"),
  blocksCount: $("#blocksCount"),
  blocksToolLabel: $("#blocksToolLabel"),
  blocksMaterialGrid: $("#blocksMaterialGrid"),
  blocksStatsCount: $("#blocksStatsCount"),
  blocksReactionCount: $("#blocksReactionCount"),
  blocksScore: $("#blocksScore"),
  blocksReactionStatus: $("#blocksReactionStatus"),
...
const state = {
  ...
  avatarTheme: ["hologram","robot","cosmic","armor"].includes(store.get("avatarTheme")) ? store.get("avatarTheme") : "hologram",
  visualTheme: ["neon","cosmic","matrix","amber","minimal"].includes(store.get("visualTheme")) ? store.get("visualTheme") : "neon",
  blocksMaterial: store.get("blocksMaterial") || "earth",
  blocksTool: ["place","remove","inspect"].includes(store.get("blocksTool")) ? store.get("blocksTool") : "place",
...
visionRenderer.setAvatarTheme(state.avatarTheme);
...
class NullHoloScene {
  ...
  setExplorerHandInteraction() {} setHandInteraction() {} setBlocksHandInteraction() {} setBlocksMaterial() {} setBlocksTool() {} setBlocksEnabled() {} seedBlocks() {} clearBlocks() {} resetBlocks() {} getBlocksSnapshot() { return { count: 0 }; } setVisualTheme() {} setFaceState() {} cancelInteraction() {}
  ...
  if (!holoScenePromise) {
    ...
    holoScenePromise = import("./three-scene.js").then(({ HoloScene }) => {
      const scene = new HoloScene($("#threeCanvas"), {
        ...
        onBlocksAction: (payload) => moduleLoader.get("blocks")?.handleSceneAction?.(payload)
      });
      scene.loaded = true;
      ...
      scene.setVisualTheme(state.visualTheme);
      scene.setBlocksMaterial(state.blocksMaterial);
      scene.setBlocksTool(state.blocksTool);
      holoScene = scene;
...
function lazyCallbacks(mode) {
  if (mode === "blocks") return {
    onState: (snapshot) => {
      const info = snapshot.materialInfo || {};
      elements.blocksMaterialName.textContent = info.label || "Bloco";
      elements.blocksCount.textContent = `${snapshot.count || 0} BLOCOS`;
      elements.blocksToolLabel.textContent = (snapshot.toolInfo?.label || "Construir").toUpperCase();
      elements.blocksStatsCount.textContent = String(snapshot.count || 0);
      elements.blocksReactionCount.textContent = String(snapshot.reactions || 0);
      elements.blocksScore.textContent = String(snapshot.score || 0);
      elements.score.textContent = (snapshot.score || 0).toLocaleString("pt-BR");
      elements.combo.textContent = `${snapshot.combo || 0}×`;
    },
    onAction: ({ action, material, score, combo }) => {
      if (action === "place") audio.select();
      if (action === "remove") audio.miss();
      elements.blocksHint.textContent = action === "place" ? `${material} adicionado` : action === "remove" ? `${material} removido` : elements.blocksHint.textContent;
      elements.score.textContent = (score || 0).toLocaleString("pt-BR"); elements.combo.textContent = `${combo || 0}×`;
    },
    onReaction: ({ reaction }) => { audio.success(); reactiveBurst("block"); elements.blocksReactionStatus.textContent = reaction?.label || "Reação descoberta"; showToast(reaction?.label || "Reação descoberta"); },
    onInspect: ({ material, materialInfo }) => { elements.blocksReactionStatus.textContent = materialInfo ? `${materialInfo.icon} ${materialInfo.label} · categoria ${materialInfo.category}` : `Material: ${material}`; },
    onXp: (xp, reason) => progression.award(xp, reason, { module: "blocks" })
  };
...
async function activateLazyMode(mode) {
  ...
  if (mode === "blocks") Object.assign(context, { scene: holoScene, material: state.blocksMaterial, tool: state.blocksTool });
...
async function applyPerformanceConfiguration({ restartCamera = false, announce = false } = {}) {
  ...
  visionRenderer.setAvatarTheme(state.avatarTheme);
  holoScene.setVisualTheme?.(state.visualTheme);
...
function handleVisionResults(raw) {
  ...
  if (hand?.[8]) {
    ...
    if (state.mode === "blocks") moduleLoader.get("blocks")?.update?.({ gestures: state.latestDisplay.gestures });
...
async function switchMode(mode, announce = true) {
  ...
  elements.blocksPanel.classList.toggle("hidden", mode !== "blocks");
  ...
  if (["explorer", "sandbox", "blocks", "face"].includes(mode)) await ensureHoloScene();
  if (mode === "blocks") moduleLoader.get("blocks")?.start?.({ scene: holoScene, material: state.blocksMaterial, tool: state.blocksTool });
  ...
  if (mode === "blocks") elements.time.textContent = "CONSTRUÇÃO";
...
function renderLoop(now) {
  ...
  if (state.started) {
    ...
    if (state.mode === "blocks") moduleLoader.get("blocks")?.update?.({ gestures: state.latestDisplay.gestures }, now);
