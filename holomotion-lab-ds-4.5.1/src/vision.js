...
export class VisionEngine {
  ...
  async startCamera(options = {}) {
    ...
    const configured = options.video || options.constraints || {};
    const frameRate = Number(options.frameRate || configured.frameRate?.ideal || profile.camera.frameRate || 30);
    const videoConstraints = {
      facingMode: options.deviceId || configured.deviceId ? undefined : (configured.facingMode || "user"),
      deviceId: options.deviceId ? { exact: options.deviceId } : configured.deviceId,
      width: configured.width || { ideal: Number(options.width || profile.camera.width) },
      height: configured.height || { ideal: Number(options.height || profile.camera.height) },
      frameRate: configured.frameRate || { ideal: frameRate, max: frameRate }
    };
    Object.keys(videoConstraints).forEach((key) => videoConstraints[key] == null && delete videoConstraints[key]);
    ...
    this.lastVideoTime = -1;
    this.lastRun = { hand: 0, pose: 0, face: 0 };
    this.onStatus({ camera: "on", settings: track?.getSettings?.(), capabilities: track?.getCapabilities?.(), label: track?.label || "" });
    this.scheduleLoop();
  }
  ...
  processMainTask(task, bitmap, timestamp) {
    ...
    if (task === "hand") {
      ...
      payload = {
        ...
        faceMatrices: (result.facialTransformationMatrixes || []).map((matrix) => {
